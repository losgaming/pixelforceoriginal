...
public class AmplifyOcclusionBase : MonoBehaviour
{
	...
	private void releaseTemporalRT()
	{
		if( m_temporalAccumRT != null )
		{
			...
			{
				safeReleaseRT( ref m_temporalAccumRT[ i ] );
			}
		}
	...
	private void checkParamsChanged()
	{
		...
		if( m_temporalAccumRT != null )
		{
			if( isStereoMultiPassEnabled() == true )
			{
		...
		if( m_temporalAccumRT == null && FilterEnabled )
		{
			...
			for( int i = 0; i < m_temporalAccumRT.Length; i++ )
			{
				m_temporalAccumRT[ i ] = safeAllocateRT( "_AO_TemporalAccum_" + i.ToString(),
														m_target.width,
														m_target.height,
	...
	{
		int tmpRT = safeAllocateTemporaryRT( cb, "_AO_IntensityTmp",
		...
		int tmpBlurRT = safeAllocateTemporaryRT( cb, "_AO_BlurIntensityTmp",
													aSourceWidth / 4, aSourceHeight / 4,
													m_motionIntensityRTFormat,
													RenderTextureReadWrite.Linear,
													FilterMode.Bilinear );
		...
		// Horizontal
		cb.SetGlobalTexture( PropertyID._AO_Source, tmpRT );
	...
	void commandBuffer_TemporalFilterDirectionsOffsets( CommandBuffer cb )
	{
	...
	void commandBuffer_FillApplyDeferred( CommandBuffer cb, bool logTarget )
	{
		...
		if( !logTarget )
		{
			if( UsingTemporalFilter )
			{
				...
				if( UsingMotionVectors == true )
				{
					safeReleaseTemporaryRT( cb, tmpMotionIntensityRT );
				}
			}
			else
			{
	...
	{
	#if UNITY_EDITOR
		return m_targetCamera.stereoEnabled && ( PlayerSettings.stereoRenderingPath == StereoRenderingPath.MultiPass );
	#else

		#if UNITY_2017_2_OR_NEWER && !UNITY_SWITCH && !UNITY_XBOXONE && !UNITY_PS4
			return	m_targetCamera.stereoEnabled && ( UnityEngine.XR.XRSettings.eyeTextureDesc.vrUsage == VRTextureUsage.OneEye );
		#else
			return	false;
		#endif
	...
	void UpdateGlobalShaderConstants_AmbientOcclusion( CommandBuffer cb )
	{
		...
		{
			commandBuffer_TemporalFilterDirectionsOffsets( cb );
		}
		else
		{
	...
	private static class ShaderPass
	{
		...
		public const int NeighborMotionIntensity = 17;
		public const int ClearTemporal = 18;
